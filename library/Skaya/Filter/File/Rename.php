 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Skaya_Filter_File_Rename implements Zend_Filter_Interface
{
	/**
	...
	public function getNewName($value, $source = false)
	{
		...
		$canvas = new Imagick($file['source']);
		
		$ext = $canvas->getImageFormat();
		if ( $ext == 'JPEG' )
			$ext = 'jpg';

		do 
			$file['target'] = dirname($file['target']) ."/". $this->getId().".".strtolower($ext);
		while 
